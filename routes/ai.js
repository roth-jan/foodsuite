...
function getTenantId(req, res, next) {
    ...
    if (!tenant && tenantKey === 'demo') {
        console.log('🏢 Auto-creating demo tenant...');
        tenant = {
            id: 1, // Use numeric ID to match existing recipes
            tenant_key: 'demo',
        ...
        // Also ensure demo data exists
        initializeDemoData(tenant.id);
    }
...
// Initialize demo data for tenant
function initializeDemoData(tenantId) {
    console.log('📝 Initializing demo data for tenant:', tenantId);
    
    // Initialize arrays if they don't exist
    if (!db.data.recipes) db.data.recipes = [];
    if (!db.data.products) db.data.products = [];
    if (!db.data.suppliers) db.data.suppliers = [];
    
    // Check if demo data already exists
    const existingRecipes = db.data.recipes.filter(r => r.tenant_id === tenantId);
    if (existingRecipes.length > 0) {
        console.log('✅ Demo data already exists');
        return;
    }
    
    // Add sample recipes for the demo tenant
    const demoRecipes = [
        {
            id: 'recipe-001',
            tenant_id: tenantId,
            ...
            ingredients: [
                ...
                { name: 'Hackfleisch', quantity: 300, unit: 'g' },
        ...
        },
        {
            ...
            cooking_time: 25,
            ingredients: [
                { name: 'Schnitzel', quantity: 4, unit: 'Stück' },
                { name: 'Kartoffeln', quantity: 800, unit: 'g' },
                { name: 'Panade', quantity: 200, unit: 'g' }
            ],
        ...
        },
        {
            id: 'recipe-003',
            tenant_id: tenantId,
            name: 'Gulasch mit Reis',
            category: 'Hauptgericht',
            servings: 4,
            cost_per_serving: 3.80,
            cooking_time: 90,
            ingredients: [
                { name: 'Rindfleisch', quantity: 600, unit: 'g' },
        ...
        },
        {
            ...
            category: 'Hauptgericht',
            servings: 4,
            cost_per_serving: 3.90,
            cooking_time: 35,
            ingredients: [
                { name: 'Hähnchenbrust', quantity: 500, unit: 'g' },
        ...
        },
        {
            ...
            cooking_time: 20,
            ingredients: [
                { name: 'Pizzateig', quantity: 400, unit: 'g' },
    ...
    console.log(`✅ Added ${demoRecipes.length} demo recipes`);
}
...
router.post('/suggest-meals', async (req, res) => {
    try {
        ...
        let recipes;
        if (process.env.DB_TYPE === 'postgres') {
            // Use PostgreSQL adapter method
            recipes = await db.getRecipes(req.tenantId, { limit: 1000 });
        } else {
            // Use in-memory database
            recipes = db.data.recipes.filter(r => r.tenant_id === req.tenantId && r.is_active);
        }
        ...
        let recipeIngredients, products;
        if (process.env.DB_TYPE === 'postgres') {
            // For PostgreSQL, skip nutritional calculation for now
            recipeIngredients = [];
            products = [];
        } else {
            recipeIngredients = db.data.recipe_ingredients || [];
...
function generateMealPlan(recipes, mode, currentPlan = {}, customConfig = null) {
    ...
    // Separate recipes by category - handle both English and German categories
    const breakfastRecipes = recipes.filter(r => 
        r.category === 'breakfast' || r.category === 'Frühstück'
    );
    const lunchRecipes = recipes.filter(r => 
        r.category === 'lunch' || r.category === 'Mittagessen' || r.category === 'Hauptgericht'
    );
    const dinnerRecipes = recipes.filter(r => 
        r.category === 'dinner' || r.category === 'Abendessen' || r.category === 'Hauptgericht'
    );
        ...
        meals.forEach(meal => {
            ...
            if (selectedRecipe) {
                plan[slot] = {
                    ...
                    cost_per_portion: selectedRecipe.cost_per_portion || selectedRecipe.cost_per_serving || selectedRecipe.cost || 3.50,
                    portions: selectedRecipe.portions || selectedRecipe.servings || 50
                };
...
function selectCostOptimized(recipes, usedRecipes) {
    ...
    // Sort by cost per portion (handle both property names)
    available.sort((a, b) => {
        const costA = a.cost_per_portion || a.cost_per_serving || 0;
        const costB = b.cost_per_portion || b.cost_per_serving || 0;
        return costA - costB;
    });
...
function calculatePlanStatistics(mealPlan, recipes) {
    ...
    Object.values(mealPlan).forEach(meal => {
        if (meal && meal.id) {
            ...
            if (recipe) {
                totalCost += (recipe.cost_per_portion || recipe.cost_per_serving || 0);
...
function optimizeForCost(plan, currentRecipes, allRecipes, suggestions) {
    ...
    currentRecipes.forEach((recipe, slot) => {
        if ((recipe.cost_per_portion || recipe.cost_per_serving || 0) > costThreshold) {
            // Find cheaper alternative
            const mealType = slot.split('-')[1];
            const alternatives = allRecipes.filter(r => 
                r.id !== recipe.id &&
                (r.category === mealType || r.category === recipe.category) &&
                (r.cost_per_portion || r.cost_per_serving || 0) < (recipe.cost_per_portion || recipe.cost_per_serving || 0)
            );
            
            if (alternatives.length > 0) {
                // Sort by cost
                alternatives.sort((a, b) => (a.cost_per_portion || a.cost_per_serving || 0) - (b.cost_per_portion || b.cost_per_serving || 0));
                ...
                suggestions.push({
                    slot,
                    reason: `Ersetzt ${recipe.name} (€${(recipe.cost_per_portion || recipe.cost_per_serving)?.toFixed(2)}) mit ${cheaperRecipe.name} (€${(cheaperRecipe.cost_per_portion || cheaperRecipe.cost_per_serving)?.toFixed(2)})`,
                    saving: ((recipe.cost_per_portion || recipe.cost_per_serving || 0) - (cheaperRecipe.cost_per_portion || cheaperRecipe.cost_per_serving || 0)).toFixed(2)
                });
            }
        }
    });
...
function selectCustomMode(recipes, usedRecipes, mealType, config) {
    ...
    const scored = available.map(recipe => {
        ...
        const nutrition = recipe.nutrition || {};
        const cost = recipe.cost_per_portion || recipe.cost_per_serving || 0;
